...
async function startTop10(){
  if(meta.variant==='host'&&!meta.hostUid){alert('اربطي جوال الهوست أولًا، أو أنشئي غرفة بدون هوست.');return;}
...
async function submitTop10(s){const input=$("topAnswer").value.trim();if(!input)return;$("topAnswer").value='';const adm=(await get(roomRef("adminPrivate/top10Current"))).val();if(!adm)return;if(meta.variant==='auto'){const rev=Object.keys(s.revealed||{}).map(Number);const b=bestTop10Match(input,adm.answers,rev);if(isAutoAccept(b))await applyTopCorrect(b.index,adm.answers[b.index]);else await applyTopWrong();}else{await set(roomRef("hostPending"),{text:input,activeTeam:s.activeTeam,submittedAt:now()});await update(roomRef("public/top10"),{lastMessage:'⏳ بانتظار تأكيد الهوست…'});}}
...
// host pairing
function listenHostPair(){onValue(roomRef("hostJoin"),async s=>{if(!hostToken||Date.now()>hostTokenExpiry)return;const reqs=s.val()||{};for(const [uid,r] of Object.entries(reqs)){if(r?.token===hostToken){await update(roomRef("meta"),{hostUid:uid});await remove(roomRef("hostJoin"));hostToken=null;const info=$("pairInfo");if(info)info.textContent='✅ تم ربط جوال الهوست.';const box=$("hostPairBox");if(box)box.classList.add('hidden');break;}}});}
...
function renderKalim(k){
  ...
  document.querySelectorAll('[data-host-card]').forEach(b=>b.onclick=e=>handleHostCardTap(+b.dataset.hostCard,e));document.querySelectorAll('[data-host-slot]').forEach(b=>b.onclick=()=>hostPlaySlot(+b.dataset.hostSlot));
  const bell=$("hostBell"),draw=$("hostDraw");if(bell)bell.onclick=hostBell;if(draw)draw.onclick=hostDraw;
}
let lastHostCardTap={i:null,at:0};
function handleHostCardTap(i,e){if(lastKalimState?.currentUid!==user.uid||lastKalimState?.transitionAt)return;const t=Date.now();if(lastHostCardTap.i===i&&t-lastHostCardTap.at<=360){e?.preventDefault?.();lastHostCardTap={i:null,at:0};hostFlipCard(i);return;}lastHostCardTap={i,at:t};hostSelectedCard=i;renderKalim(lastKalimState);}
async function hostFlipCard(i){await runTransaction(roomRef("public/kalim"),k=>{if(!k||k.currentUid!==user.uid||k.transitionAt)return k;const c=k.hands?.[user.uid]?.[i];if(c)c.useBack=!c.useBack;return k;});hostSelectedCard=i;}
async function hostPlaySlot(slot){if(lastKalimState?.currentUid!==user.uid||lastKalimState?.transitionAt)return;if(hostSelectedCard==null){await hostUndoSlot(slot);return;}let starLetter=null;const c=lastKalimState?.hands?.[user.uid]?.[hostSelectedCard];if(activeFace(c)==='★'){starLetter=prompt('النجمة تمثل أي حرف؟ اكتب حرفًا عربيًا واحدًا:')?.trim();if(!starLetter)return;}const idx=hostSelectedCard;hostSelectedCard=null;await runTransaction(roomRef("public/kalim"),k=>{if(!k||k.currentUid!==user.uid||k.transitionAt)return k;const hand=k.hands?.[user.uid]||[];const card=hand[idx];if(!card)return k;const f=card.useBack?card.b:card.a,o=card.useBack?card.a:card.b;hand.splice(idx,1);k.stacks[slot].push({letter:f==='★'?starLetter:f,other:o,ownerUid:user.uid,card});k.lastAction=`${meta.ownerName||'الهوست'} وضع حرف ${f==='★'?starLetter:f}`;if(hand.length===0)k.winnerUid=user.uid;return k;});}
async function hostUndoSlot(slot){await runTransaction(roomRef("public/kalim"),k=>{if(!k||k.currentUid!==user.uid||k.transitionAt)return k;const st=k.stacks?.[slot];if(!st||st.length<=1)return k;const top=st.pop();if(top.ownerUid&&top.card){k.hands[top.ownerUid]=k.hands[top.ownerUid]||[];k.hands[top.ownerUid].push(top.card);k.lastAction=`تم التراجع عن حرف ${top.letter}`;}return k;});}
async function hostDraw(){await runTransaction(roomRef("public/kalim"),k=>{if(!k||k.currentUid!==user.uid||k.transitionAt)return k;const c=k.deck?.pop();if(c){k.hands[user.uid].push(c);k.lastAction=`${meta.ownerName||'الهوست'} سحب بطاقة`;}return k;});}
async function hostBell(){await runTransaction(roomRef("public/kalim"),k=>{if(!k||k.currentUid!==user.uid)return k;const n=Date.now();if(!k.bellStopped){k.remainingMs=k.timerRunning?Math.max(0,(k.deadline||n)-n):(k.remainingMs||0);k.timerRunning=false;k.bellStopped=true;k.transitionAt=n+2000;k.lastAction=`${meta.ownerName||'الهوست'} أنهى دوره`;return k;}const ms=k.timerMs||14000;k.currentIndex=((k.currentIndex||0)+1)%k.order.length;k.currentUid=k.order[k.currentIndex];k.remainingMs=ms;k.deadline=n+ms;k.timerRunning=true;k.bellStopped=false;k.transitionAt=null;k.lastAction='بدأ دور اللاعب التالي';return k;});}
...
async function stopExpiredKalim(){await runTransaction(roomRef("public/kalim"),k=>{if(!k)return k;const nowMs=Date.now();if(k.timerRunning&&(k.deadline||0)<=nowMs){k.timerRunning=false;k.remainingMs=0;k.transitionAt=nowMs+2000;k.bellStopped=true;k.lastAction='انتهى الوقت.';return k;}if(!k.timerRunning&&k.transitionAt&&k.transitionAt<=nowMs){const ms=k.timerMs||14000;k.currentIndex=((k.currentIndex||0)+1)%k.order.length;k.currentUid=k.order[k.currentIndex];k.remainingMs=ms;k.deadline=nowMs+ms;k.timerRunning=true;k.bellStopped=false;k.transitionAt=null;k.lastAction='بدأ دور اللاعب التالي تلقائيًا.';}return k;});}
async function toggleKalimPause(){await runTransaction(roomRef("public/kalim"),k=>{if(!k||k.transitionAt)return k;if(k.timerRunning){k.remainingMs=Math.max(0,(k.deadline||Date.now())-Date.now());k.timerRunning=false;k.lastAction='تم إيقاف الوقت.';}else{k.deadline=Date.now()+(k.remainingMs||k.timerMs||14000);k.timerRunning=true;k.lastAction='تم استكمال الوقت.';}return k;});}
async function resetKalimTimer(){await runTransaction(roomRef("public/kalim"),k=>{if(!k)return k;const ms=k.timerMs||14000;k.remainingMs=ms;k.deadline=Date.now()+ms;k.timerRunning=true;k.bellStopped=false;k.transitionAt=null;k.lastAction=`أُعيد الوقت إلى ${Math.round(ms/1000)} ثانية.`;return k;});}
async function returnKalimTurn(){const k=(await get(roomRef("public/kalim"))).val();if(!k)return;const names=Object.fromEntries(allMembers().map(([u,m])=>[u,m.name]));const choices=(k.order||[]).map((u,i)=>`${i+1}: ${names[u]||u}`).join('\n');const n=prompt('اختاري رقم اللاعب:\n'+choices);const idx=Number(n)-1;if(idx<0||idx>=k.order.length)return;await update(roomRef("public/kalim"),{currentIndex:idx,currentUid:k.order[idx],remainingMs:k.timerMs||14000,deadline:Date.now()+(k.timerMs||14000),timerRunning:true,bellStopped:false,transitionAt:null,lastAction:`أُعيد الدور إلى ${names[k.order[idx]]||''}`});}
setInterval(async()=>{if(meta?.game==='kalim'&&meta?.status==='playing'){const s=(await get(roomRef("public/kalim"))).val();const el=$("displayTimer");if(s){await stopExpiredKalim();if(el)el.textContent=Math.ceil(remaining(s)/1000);}}},250);
